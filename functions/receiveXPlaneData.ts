...
const latestPayload = new Map();       // flight.id → aktuellste Payload
...
Deno.serve(async (req) => {
  try {
    ...
    const {
      ...
      total_weight_kg, oat_c, ground_elevation_ft, baro_setting,
      wind_speed_kts, wind_direction, aircraft_icao, fms_waypoints
    } = data;
    ...
    latestPayload.set(flight.id, xplaneData); // **Immer aktuell**

    // --------- Async Flight Update (DB) ---------
    ...
    // --------- Response: Immer aktuell zurückgeben ---------
    ...
    return Response.json({
      ...
      xplane_data: xplaneData   // **Immer die aktuellste Payload**
    });
